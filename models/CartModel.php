	...
	trait CartModel{		
		public function cartAdd($id){
		    ...
		    if(isset($_SESSION['cart'][$id])){
				...
				$name ='product_'.$_SESSION['cart']['number']; 
		...
		public function cartCheckOut(){
			...
			// echo $price;
			$query = $conn->prepare("insert into orders set customer_id=:customer_id,name=:name,phone=:phone,address=:address,pay=:pay, date=now(),price=:price");
			$query->execute(["customer_id"=>$customer_id,"price"=>$price,"address"=>$address,"phone"=>$phone,"name"=>$name,"pay"=>$pay]);
			// lay id vua moi insert
			...
			foreach($_SESSION["cart"] as $product){
				$query = $conn->query("update products set  quantity = quantity - {$product['number']} where id={$product['id']}");
			}
			//xoa gio hang
			unset($_SESSION["cart"]);
		}
		public function modelQuantityProduct()
		{
			// $id = $_SESSION['cart']['id'];
			$conn = Connection::connectDB();
			foreach($_SESSION["cart"] as $product){
			$query = $conn->query("select quantity from products where id = {$product['id']}");
